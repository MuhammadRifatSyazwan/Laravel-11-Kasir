...
class Transaksi extends Component
{
    ...
    public function updatedKode()
    {
        ...
        if ($produk && $produk->stok > 0) {
            // Cari atau buat DetilTransaksi baru untuk transaksi dan produk yang diberikan
            $detil = DetilTransaksi::firstOrNew(
                ...
                [
                    'jumlah' => 0, // Default jumlah jika DetilTransaksi baru dibuat
                ]
            );

            // Tambahkan jumlah produk di DetilTransaksi
            $detil->jumlah += 1;
            // $detil->save(); // Simpan perubahan ke database

            // Kurangi stok produk
            $produk->stok -= 1;
            $produk->save(); // Simpan perubahan stok ke database

            // Reset input kode (mengatur ulang variabel 'kode')
            $this->reset('kode');
        }
